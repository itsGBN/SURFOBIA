...
public class MainMenuEvents : MonoBehaviour
{
    ...
    private void Update()
    {
        if ((GetInputs.PS5Map.Menu.WasPressedThisFrame() || GetInputs.PS5Map.Jump.WasPressedThisFrame()) && !MainMenuEvents.instance.isTrasitioning && (GameManager.instance.gameState == GameManager.GameState.READY))
        {
            switch (dpadnum) 
            {
                case 0:
